<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>CHURCH</title>
    <link rel="shortcut icon" href="asess/icons/logo.svg" type="image/x-icon">
    <link rel="stylesheet" href="asess/vendor/bootstrap 4/css/bootstrap.min.css">
    <link rel="stylesheet" href="asess/vendor/mdb/css/mdb.min.css">
    <link rel="stylesheet" href="asess/vendor/font-awesome 4/css/font-awesome.min.css">
    <link rel="stylesheet" href="asess/css/main.css">
    <link rel="stylesheet" href="asess/css/animations.css">
    <link rel="stylesheet" href="asess/css/reponsible.css">
    <link rel="stylesheet" href="asess/vendor/lightbox/css/lightbox.min.css">
</head>

<body>
    <!-- <div class="loading-panel"></div> -->

    <header class="header mb-5" id="header">
        <div class="top">
            <div class="info wow fadeLeft">
                <p class="mr-2 mt-3 py-0"><i class="fa fa-map-marker" aria-hidden="true"></i> 345 santa isabell
                    treet,CZA 90011</p>
                <p class="mr-2 mt-3 py-0"><i class="fa fa-phone" aria-hidden="true"></i> (+224) 900 - 000 - 111</p>
                <p class="mr-2 mt-3 py-0"><i class="fa fa-location-arrow" aria-hidden="true"></i> contact@church.com</p>
            </div>
            <div class="redes wow fadeIn">
                <i class="fa fa-facebook mx-2 mt-2 py-0" aria-hidden="true"></i>
                <i class="fa fa-twitter mx-2 mt-2 py-0" aria-hidden="true"></i>
                <i class="fa fa-whatsapp mx-2 mt-2 py-0" aria-hidden="true"></i>
                <i class="fa fa-instagram mx-2 mt-2 py-0" aria-hidden="true"></i>
            </div>
        </div>
        
        <?
            require_once("./components/page-header.php");
        ?>

        <div class="banner-texts">
            <div class="banner-text text-white">
                <div>
                    <h2>online sermons each sunday night</h2>
                    <p>what are you complaining about? said god. <br> they wo'nt let me in either</p>
                </div>
                <button type="button" class="btn btn-style">all sermons</button>
            </div>
        </div>
    </header>
    <!-- recents events -->
    <div class="container recent-events pb-5">
        <div class="row">
            <div class="col-xl-12 text-center">
                <article class="event-text">
                    <h2>Recent events</h2>
                <p>
                    Lorem ipsum dolor sit amet, consectetur adipisicing elit. Excepturi, illum?
                </p>
                </article>
            </div>
        </div>
        <div class="row d-flex justify-align-center align-items-center">
            <div class="col-xl-8 col-lg-8 col-md-6 event-card mb-5">
                <div class="row py-3">
                    <div class="col-xl-4 col-lg-4 recent-img">
                        <img src="asess/img's/sermon-article-3.png" class="img-fluid" alt="">
                    </div>
                    <div class="col-xl-8 col-lg-8 col-md-12">
                        <article>
                            <h3>Recents events</h3>
                            <p>
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor repudiandae illo possimus cupiditate saepe.
                            </p>
                            <button type="button" class="btn btn-style-2 ml-0">view event</button>
                        </article>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-4 event-line">
                <article>
                    <p>15 Jully 2021</p>
                    <h3>THU 01:30</h3>
                </article>
            </div>
        </div>

        <div class="row d-flex justify-align-center align-items-center">
            <div class="col-xl-8 col-lg-8 col-md-6 event-card">
                <div class="row py-3">
                    <div class="col-xl-4 col-lg-4 recent-img">
                        <img src="asess/img's/sermon-article-3.png" class="img-fluid" alt="">
                    </div>
                    <div class="col-xl-8 col-lg-8 col-md-12">
                        <article>
                            <h3>Recents events</h3>
                            <p>
                                Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolor repudiandae illo possimus cupiditate saepe.
                            </p>
                            <button type="button" class="btn btn-style-2 ml-0">view event</button>
                        </article>
                    </div>
                </div>
            </div>
            <div class="col-xl-4 col-lg-4 event-line">
                <article>
                    <p>15 Jully 2021</p>
                    <h3>THU 01:30</h3>
                </article>
            </div>
        </div>
    </div>
    <!-- video content -->
    <div class="video-content">
        <div class="icon-play">
            <img src="asess/icons/play-icon.svg" alt="play-icon">
        </div>
        <img src="asess/img's/sermon-article-2.png" alt="">
    </div>
    <!-- about the church -->
    <div class="container-fluid">
        <div class="container px-0">
            <div class="row py-5">
                <div class="col-xl-12 text-center">
                    <h2>About the Church</h2>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut
                        seedor labore. Excepteur <br> sint occaecat.
                    </p>
                </div>
            </div>
            <div class="row d-flex justify-content-center align-items-center">
                <div class="col-xl-6 col-md-6 col-sm-12" id="none">
                    <img src="asess/img's/er.png" class="img-fluid"  alt="">
                </div>
                <div class="col-xl-6 col-md-6 col-sm-12">
                    <article class="mb-5">
                        <p>We love God. We beliave in God.</p>
                        <h3>Welcome to Cristian Church</h3>
                        <p>
                            Lorem ipsum, dolor sit amet consectetur adipisicing elit. Doloribus nulla corrupti quam
                            dolores quod! Asperiores, magni quasi.
                        </p>
                    </article>
                    <div class="row">
                        <div class="col-xl-6 col-md-6 col-sm-12 py-3">
                            <h4>Our mission</h4>
                            <p>
                                Our mission is what we do. The active verb here is invite. God progresses through
                                gracious invitations.
                            </p>
                        </div>
                        <div class="col-xl-6 col-md-6 col-sm-12 py-3">
                            <h4>Our vision</h4>
                            <p>
                                we see our mission as extending God's Kingdom and we seek to help every member find
                                their place.
                            </p>
                        </div>
                        <div class="col-xl-6 col-md-6 col-sm-12 py-3">
                            <h4>Our values</h4>
                            <p>
                                community is where we most deeply experience God's and grace throughout all stages of
                                life.
                            </p>
                        </div>
                        <div class="col-xl-6 col-md-6 col-sm-12 py-3">
                            <h4>Our faith</h4>
                            <p>
                                All peoples are of tremendous value to God; reaching them with the Gospel is our
                                district responsibility.
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- porcents -->
    <div class="container-fluid porcents">
        <div class="row">
            <div class="col-xl-3 col-lg-3 col-md-6">
                <article>
                    <img src="asess/icons/people.png" alt="">
                    <h3>823</h3>
                    <p>people</p>
                </article>
            </div>
            <div class="col-xl-3 col-lg-3 col-md-6">
                <article>
                    <img src="asess/icons/heart.png" alt="">
                    <h3>241</h3>
                    <p>donations</p>
                </article>
            </div>
            <div class="col-xl-3 col-lg-3 col-md-6">
                <article>
                    <img src="asess/icons/bible.png" alt="">
                    <h3>186</h3>
                    <p>sermons</p>
                </article>
            </div>
            <div class="col-xl-3 col-lg-3 col-md-6">
                <article>
                    <img src="asess/icons/hands.png" alt="">
                    <h3>342</h3>
                    <p>volunteers</p>
                </article>
            </div>
        </div>
    </div>

    <!-- our galery -->

    <div class="container-fluid galery">
        <div class="container pb-5">
            <div class="row">
                <div class="col-xl-12 text-center py-5">
                    <h2>Our Galery</h2>
                    <p>
                        Lorem ipsum dolor, sit amet consectetur adipisicing elit. Quis, eius.
                    </p>
                </div>
            </div>

            <div class="row b-5">
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/imglight-1.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/imglight-1.jpg" class="img-fluid" alt="">
                </div>
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/imglight-2.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/imglight-2.jpg" class="img-fluid" alt="">
                </div>
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/imglight-3.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/imglight-3.jpg" class="img-fluid" alt="">
                </div>
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/new4.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/new4.jpg" class="img-fluid" alt="">
                </div>
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/new6.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/new6.jpg" class="img-fluid" alt="">
                </div>
                <div class="col-md-4 my-img-card-style">
                    <div class="hoverly">
                        <a href="asess/img/new7.jpg" class="img-link" data-lightbox="exemple-set" data-title="DevCode">
                            <i class="fa fa-plus fa-5x text-white" aria-hidden="true"></i>
                        </a>
                    </div>
                    <img src="asess/img/new7.jpg" class="img-fluid" alt="">
                </div>
            </div>
        </div>
    </div>

    <!-- our sermons -->

    <div class="container-fluid sermons mb-5">
        <div class="container">
            <div class="row">
                <div class="col-xl-12 text-center py-5">
                    <h2>Our Sermons</h2>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Consequuntur, cum?
                    </p>
                </div>
            </div>
            <div class="row cards-sermon">
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 mb-4">
                    <div class="ser-card">
                        <div class="img-ser">
                            <img src="asess/img's/sermon-article-3.png" alt="">
                        </div>
                        <div class="ser-text">
                            <h4>Let's pray for Kongo kids</h4>
                            <p>By Crisvan dos Santos&nbsp;|&nbsp;jully 15,2021</p>
                        </div>
                        <div class="ser-redes">
                            <p>
                                <img src="asess/icons/share.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/video.png" alt="">
                            </p>
                            <p>
                                <img src="asess/icons/download.png" alt="">
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <button type="button" class="btn btn-style-2 ml-0">view sermons</button>
        </div>
    </div>

    <!-- Our blog -->

    <div class="container-fluid blog">
        <div class="container">
            <div class="row pb-5 d-flex align-items-center justify-content-center">
                <div class="col-xl-12 text-center">
                    <h2>Our Blog</h2>
                    <p>
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Unde, tenetur.
                    </p>
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-center">
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 mb-4">
                    <div class="blog-card">
                        <div class="img-blog-card">
                            <img src="asess/img's/banner.png" alt="">
                        </div>
                        <div class="text-blog-card">
                            <div class="date-text">
                                <p>15<br>Jully</p>
                            </div>
                            <div class="text-content">
                                <h3>Why holiday churches are so small</h3>
                                <p class="pb-0 mb-0">
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Magni, rerum repellendus
                                    possimus, eligendi odio architecto quas beatae.
                                </p>
                                <a href="#">Read More <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 mb-4">
                    <div class="blog-card">
                        <div class="img-blog-card">
                            <img src="asess/img's/banner.png" alt="">
                        </div>
                        <div class="text-blog-card">
                            <div class="date-text">
                                <p>15<br>Jully</p>
                            </div>
                            <div class="text-content">
                                <h3>Why holiday churches are so small</h3>
                                <p class="pb-0 mb-0">
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Magni, rerum repellendus
                                    possimus, eligendi odio architecto quas beatae.
                                </p>
                                <a href="#">Read More <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 mb-4">
                    <div class="blog-card">
                        <div class="img-blog-card">
                            <img src="asess/img's/banner.png" alt="">
                        </div>
                        <div class="text-blog-card">
                            <div class="date-text">
                                <p>15<br>Jully</p>
                            </div>
                            <div class="text-content">
                                <h3>Why holiday churches are so small</h3>
                                <p class="pb-0 mb-0">
                                    Lorem ipsum dolor sit amet consectetur adipisicing elit. Magni, rerum repellendus
                                    possimus, eligendi odio architecto quas beatae.
                                </p>
                                <a href="#">Read More <i class="fa fa-long-arrow-right" aria-hidden="true"></i></a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="row d-flex align-items-center justify-content-center">
                <button type="button" class="btn mt-3 btn-style-2">View Sermons</button>
            </div>
        </div>
    </div>

    <!-- footer side -->
    <div class="container-fluid px-0">
        <div class="newlatter">
            <h2>NEWLATTER</h2>
            <form action="#">
                <input type="text" name="email" id="email" class="new-input" placeholder="Subscrible your email here">
                <button type="#" class="form-btn ml-0"><i class="fa fa-location-arrow" aria-hidden="true"></i></button>
            </form>
        </div>
    </div>

    <footer class="container-fluid mt-0 py-5 text-white footer">
        <div class="container">
            <div class="row align-items-center justify-content-center">
                <div class="col-xl-3 col-md-3 col-sm-6">
                    <article class="foot-text-area">
                        <div class="foot-header">
                            <div class="img-fo">
                                <img src="asess/icons/logo.svg" alt="logo">
                            </div>
                            <h3>CHURCH OF CRISTIAN</h3>
                        </div>
                        <P>
                            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quidem error velit hic!
                        </P>
                        <div class="fot-redes">
                            <i class="fa fa-facebook-square" aria-hidden="true"></i>
                            <i class="fa fa-twitter" aria-hidden="true"></i>
                            <i class="fa fa-whatsapp" aria-hidden="true"></i>
                            <i class="fa fa-instagram" aria-hidden="true"></i>
                        </div>
                    </article>
                </div>
                <div class="col-xl-3 col-md-3 col-sm-6">
                    <article class="foot-contact">
                        <h3 class="mb-3">Contactos</h3>
                        <small class="cont">
                            <div class="d-flex">
                                <i class="fa fa-phone fa-lg mr-2" aria-hidden="true"></i>
                                <p>(+224) 900 - 000 - 111 <br> (+224) 900 - 000 - 111</p>
                            </div>
                            <div class="d-flex">
                                <i class="fa fa-envelope-o fa-lg mr-2" aria-hidden="true"></i>
                                <p>contact@church.com <br> contact@church.com</p>
                            </div>
                            <div class="d-flex">
                                <i class="fa fa-map-marker fa-lg mr-2" aria-hidden="true"></i>
                                <p>345 Santa Isabell Street CZA 9001</p>
                            </div>
                        </small>
                    </article>
                </div>
                <div class="col-xl-3 col-md-3 col-sm-6">
                    <article class="useful-area">
                        <h3>Useful Links</h3>
                        <small>
                            <p><i class="fa fa-angle-right" aria-hidden="true"></i> Need help? get in touch</p>
                            <p><i class="fa fa-angle-right" aria-hidden="true"></i> About our church</p>
                            <p><i class="fa fa-angle-right" aria-hidden="true"></i> Latest news</p>
                            <p><i class="fa fa-angle-right" aria-hidden="true"></i> Our event</p>
                            <p><i class="fa fa-angle-right" aria-hidden="true"></i> Our sermons</p>
                        </small>
                    </article>
                </div>
                <div class="col-xl-3 col-md-3 col-sm-6 recent-posts">
                    <h3>Recent posts</h3>
                    <div class="posts mb-3">
                        <div class="post-img">
                            <img src="asess/img's/sermon-article-4 (2).png" alt="">
                        </div>
                        <div class="post-text">
                            <h5 class="mb-0">What religion gives<br>us</h5>
                            <p>Jully 15,2021</p>
                        </div>
                    </div>
                    <div class="posts">
                        <div class="post-img">
                            <img src="asess/img's/sermon-article-4 (2).png" alt="">
                        </div>
                        <div class="post-text">
                            <h5 class="mb-0">What religion gives<br>us</h5>
                            <p>Jully 15,2021</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <div class="container-fluid py-2">
        <div class="row justify-content-center align-items-center">
            <div class="col-xl-6 text-center wow fadeIn">
                <h6 class="mt-3">CRIANT CHURCH Theme by JandyEgi. All Rights Reserverd
                </h6>
            </div>
        </div>
    </div>

    <!-- scripts -->
    <script src="asess/vendor/jquery/jquery.min.js"></script>
    <script src="asess/vendor/bootstrap 4/js/bootstrap.min.js"></script>
    <script src="asess/vendor/mdb/js/mdb.min.js"></script>
    <script src="asess/js/app.js"></script>
    <script src="asess/vendor/lightbox/js/lightbox.min.js"></script>
</body>

</html>